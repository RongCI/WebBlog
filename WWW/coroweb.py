...
class RequestHandler(object):
    def __init__(self,app,fn):
        ...
        self._requires_kw_args = get_required_kw_args(fn)

    @asyncio.coroutine
    def __call__(self,request):
        kw = None
        if self._has_var_kw_arg or self._has_named_kw_args or self._requires_kw_args:
            if request.method == 'POST':
                if not request.content_type:
                    return web.HTTPBadRequest('Missing content')
        ...
        if self._has_request_arg:
            kw['request'] = request
        if self._requires_kw_args:
            for name in self._requires_kw_args:
                if not name in kw:
